...
One of the clustering algorithms within *graph-based* approach is the **MST-kNN** [@mario]. It uses two proximity graphs: *minimum spanning tree (MST)* and *k nearest neighbor (kNN)*. They are able to model the data and highlight the more important relationships between the objects of the data. MST-kNN requires minimal user intervention due to automatic selection of the *k* value. Such situation is adequate in scenarios where the structure of the data is unknown.

This document gives a quick guide of `mstknnclust` package (version 0.1.0). It corresponds to the implementation of the MST-kNN clustering algorithm. For further details to see `help(package="mstknnclust")`. If you use this R package do not forget to include the references provided by `citation("mstknnclust")`.
...
#### Getting the results
The function `mst.knn` return a list with five elements:

1. **k**: Number of cluster of the solution.
2. **csize**: A vector with the cardinality of each cluster.
3. **cluster**: A named vector of integers from *1:k* representing the cluster to which each object is assigned.
4. **partition**: A partition matrix order by the clusters where objects were allocated.
